...
from utils.systems import Controlled3DOFAircraft

th.set_default_dtype(th.float64)
plt.ioff()


class Aircraft3DOFModel(pl.LightningModule):
    def __init__(
        self,
        X_star: th.Tensor,
        t_span: th.Tensor,
        ...
        u = NeuralController(
            X_star=X_star,
            state_dim=Controlled3DOFAircraft.STATE_DIM,
            control_dim=Controlled3DOFAircraft.CONTROL_DIM,
            hidden_dims=[32, 32, 32],
            gain=10 * pi / 180,
            modulo=Controlled3DOFAircraft.MODULO,
        )
        # Controlled system
        sys = Controlled3DOFAircraft(u=u)
    ...
    def validation_step(self, batch: th.Tensor, batch_idx: int):
        ...
        for i in range(len(x0)):
            ax.plot(t_span_fine, traj[:, i, 0], "k", alpha=0.3, label="a" if i == 0 else None)
...
if __name__ == "__main__":
    ...
    model = Aircraft3DOFModel(X_star=X_star, t_span=t_span, max_epochs=max_epochs, lr=lr, reg_coef=reg_coef, P=P, Q=Q)
    trainer = pl.Trainer(
